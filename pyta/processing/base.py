...
class AcquisitionProcessingMixin:
    ...
    def initialise_arrays(
        probe_array: np.ndarray, reference_array: np.ndarray, first_pixel: int, num_pixels: int
    ...
    def separate_on_off(
        ...
        if (untrimmed_probe_array[0, trigger_pixel] >= trigger_threshold and not tau_flip_request) or (
            untrimmed_probe_array[0, trigger_pixel] < trigger_threshold and tau_flip_request
        ):
    ...
    @staticmethod
    def set_linear_pixel_correlation(raw_probe_array: np.ndarray, raw_reference_array: np.ndarray) -> LinearPixelCorrection:
    ...
    @staticmethod
    def linear_pixel_correction(
        probe_array: np.ndarray, reference_array: np.ndarray, lpc: LinearPixelCorrection
    ...
    def manipulate_reference(
        data: AcquisitionOnOffData, num_pixels: int, reference_manipulation_factors: ReferenceManipulationFactor
    ) -> AcquisitionOnOffData:
    ...
    @staticmethod
    def calculate_dtt(
        probe_on_array: np.ndarray, probe_off_array: np.ndarray, probe_off_avg: np.ndarray | None = None
    ) -> np.ndarray:
    ...
    @staticmethod
    def calculate_dtt_error(
        data: AcquisitionOnOffData, refd_probe_off_array: np.ndarray, data_avg: AcquisitionOnOffData | None = None
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        if data_avg is not None:
            probe_shot_error = np.std(2 * (data.probe_on - data.probe_off) / (data_avg.probe_on + data_avg.probe_off), axis=0)
            ref_shot_error = np.std(
                2 * (data.reference_on - data.reference_off) / (data_avg.reference_on + data_avg.reference_off), axis=0
            )
        else:
            probe_shot_error = np.std(2 * (data.probe_on - data.probe_off) / (data.probe_on + data.probe_off), axis=0)
            ref_shot_error = np.std(
                2 * (data.reference_on - data.reference_off) / (data.reference_on + data.reference_off), axis=0
            )
